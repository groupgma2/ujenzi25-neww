...
if (!url || !serviceKey) {
  console.warn('Supabase URL or service role key not configured for backend supabase client. Server-side DB operations will be disabled. Falling back to Firestore if configured.');
}
...
export async function insertMessage(payload: { sender_id: string; receiver_id?: string | null; content: string; thread_id?: string; context_type?: string; context_id?: string | null }) {
  ...
  return await insertMsgFirestore(payload);
}
...
export async function fetchMessagesByThread(threadId: string) {
  ...
  return await fetchCollection('messages', { ownerId: undefined }).then(list => list.filter(m => m.thread_id === threadId));
}
...
export async function fetchMessagesByContext(contextType: string, contextId: string) {
  ...
  return await fetchCollection('messages', { ownerId: undefined }).then(list => list.filter(m => m.context_type === contextType && m.context_id === contextId));
}
